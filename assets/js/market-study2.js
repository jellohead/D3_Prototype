var WIDTH = 800, // width of the graph
  ...
  currentDataset, // name of the current data set. Used to track when the datasetDropDown changes
...
function redraw () {
  ...
  surveyResponse.enter()
    .insert("svg:circle")
      ...
      .style("fill", function (d) {
        return colours[d.type.id];
      }); // set fill colour from the colours array
  ...
  surveyResponse.transition().duration(1500).ease("exp-in-out")
    .style("opacity", 1)
...
function generateTypesList (data) {
  ...
  console.log("Now in the generateTypesList function.");
}

// return the name of the datasetDropDown which is currently selected
function getChosenDataset () {
  //var select = document.getElementById("datasetDropDown");
  return 1;
  //select.options[select.selectedIndex].value;
  // returns value of selected item in drop down box
}
...
function getAxes () {
  var x = document.querySelector("#x-axis input:checked").value,
    ...
    r = document.querySelector("#radius input:checked").value;
  return {
    xAxis: x,
    yAxis: y,
    radiusAxis: r
...
// remove coasters whose type is not selected from a datasetDropDown
/*Commented out since this function isn't used*/
